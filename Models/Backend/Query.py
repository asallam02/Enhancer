'''This class takes in user inputs and queries the USCS database.
It can then use these results to get results from enformer and
store them. 
'''
...
class Query:
    def __init__(self, chromosome: str, start: int, end: int, enformer_model):
        ...
        # extract given DNA sequence from database
    ...
    def calculate_enformer(self):
        ...
        # get enformer results for the original DNA sequence
        self.orig_result = self.model.enform(self.seq_str)[:, self.tracks]
        
        # get enformer results for the modded DNA sequence
